(()=>{
  ...
  function branch(p,seen=new Set()){
    let k=key(p.external_id);if(seen.has(k))return'';
    let n=new Set(seen);n.add(k),s=spouses(p),c=children(p);
    let h='<div class="family"><div class="couple">'+card(p)+(s[0]?'<span>💍</span>'+card(s[0]):'')+'</div>';
    if(open.has(k)&&c.length)h+='<div class="children">'+c.map(x=>'<div class="kid">'+branch(x,n)+'</div>').join('')+'</div>';
    return h+'</div>';
  }
  ...
  function treeRender(q){
    ...
    document.getElementById('content').innerHTML='<div class="tree"><div class="hint" style="text-align:center">'+note+'</div>'+rs.map(p=>branch(p)).join('')+'</div>';
  }
  function render(){
