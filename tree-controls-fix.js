(()=>{
  ...
  let people=[],open=new Set(),view='tree',edits={},pedits={},initialized=false;
  const key=v=>{let s=String(v??'').trim().replace(/^@|@$/g,'').toLowerCase(),m=s.match(/i\d+|l\d+/i);return m?m[0].toLowerCase():s};
  const esc=v=>String(v??'').replace(/[&<>\"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#39;'}[c]));
  const GP=window.GEDCOM_PARENTS||{},GS=window.GEDCOM_SPOUSES||{};
  const ged=(m,id)=>m[id]||m[key(id)]||m['@'+key(id)+'@']||m['@'+key(id)]||[];
  function load(){
    const base=Array.isArray(window.FAMILY_MEMBERS_STATIC)?window.FAMILY_MEMBERS_STATIC:[];
    let added=[];try{added=JSON.parse(localStorage.getItem(ADDSTORE)||'[]')||[]}catch{}
    const ids=new Set(base.map(a=>key(a[0])));
    added.forEach(a=>{if(a?.external_id&&!ids.has(key(a.external_id))){base.push([a.external_id,a.full_name||'Unnamed',a.sex||'',a.birth_date||'',a.place||'',a.occupation||'']);ids.add(key(a.external_id));}});
    window.FAMILY_MEMBERS_STATIC=base;
    people=base.map(a=>({external_id:a[0],full_name:a[1],sex:a[2],birth_date:a[3],place:a[4],occupation:a[5]}));
    try{edits=JSON.parse(localStorage.getItem(STORE)||'{}')||{};pedits=JSON.parse(localStorage.getItem(PSTORE)||'{}')||{}}catch{edits={};pedits={}}
    if(!initialized){people.forEach(p=>open.add(key(p.external_id)));initialized=true;}
  }
  ...
  function roots(){let hasParents=new Set();people.forEach(p=>{if(parents(p).length)hasParents.add(key(p.external_id));});let out=[];people.filter(p=>!hasParents.has(key(p.external_id))).forEach(p=>{let ps=spouses(p);if(ps.some(s=>hasParents.has(key(s.external_id))))return;let ids=[key(p.external_id),...ps.map(x=>key(x.external_id))];if(!out.some(x=>ids.includes(key(x.external_id))))out.push(p)});let added=[];try{added=JSON.parse(localStorage.getItem(ADDSTORE)||'[]')||[]}catch{};added.forEach(a=>{let p=people.find(x=>key(x.external_id)===key(a.external_id));if(p&&!out.some(x=>key(x.external_id)===key(p.external_id)))out.push(p)});return out}
